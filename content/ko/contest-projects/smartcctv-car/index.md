...
탐지→crop→분류의 two-stage 구조로 관심 차량만 분류했습니다.

### 2. 홍보용 차량 사진과 폐차장 CCTV 사이의 도메인 차이

학습 split 수치와 현장 유사 crop 검증을 분리해 기록하고, 실제 CCTV 조건을 별도 확인했습니다.

### 3. GPU 모델을 현장 NPU에서 그대로 실행할 수 없음

장비가 지원하는 연산과 입력 규격에 맞게 모델을 변환하고 Linux 관제 프로그램에 적용했습니다.
