...
실제 관제 환경에서는 이벤트 스냅샷으로 낙상 후보와 객체 탐지 결과를 확인했습니다. 이후 낙상·비낙상 hard validation set으로 오탐과 미탐을 분리해 점검했으며, 50개의 낙상 장면과 50개의 일반 사람 장면에 대한 2차 객체 탐지 모델의 판정 분포를 비교했습니다. pose와 zero-shot 검증은 이 흐름의 앞뒤 단계에 결합했습니다.

![실제 관제 환경에서 감지한 낙상 이벤트 화면](detail-event.png)
